      ...
      <div className="grid gap-6 lg:grid-cols-12">
        <section className="lg:col-span-8">
          ...
          <div className="overflow-hidden rounded-xl border border-outline-variant/50 bg-white shadow-sm">
            {summary.jobs.recent.length ? summary.jobs.recent.map((job) => { const style = statusStyle[job.status] || statusStyle.queued; const Icon = style.icon; return <div key={job._id} className="grid grid-cols-12 items-center gap-4 border-b border-outline-variant/30 p-4 last:border-0"><div className="col-span-4 sm:col-span-3"><span className={`inline-flex items-center gap-1 rounded px-2 py-0.5 text-[10px] font-bold uppercase ${style.className}`}><Icon className={`h-3.5 w-3.5 ${job.status === 'processing' ? 'animate-spin' : ''}`} />{job.detailedStatus || job.status}</span></div><div className="col-span-5"><div className="font-mono text-[12px] font-medium text-on-surface">{job.skuId}</div><div className="mt-0.5 text-[11px] text-secondary">{job.skuName || `${job.totalPoses} pose set`}</div></div><div className="col-span-3 text-right text-[12px] text-secondary">{relativeTime(job.createdAt)}</div></div> }) : <div className="p-8 text-center text-sm text-secondary">No generation jobs yet. Start the first one in Studio.</div>}
          </div>
        </section>
