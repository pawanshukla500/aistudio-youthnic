          ...
          <div className="grid gap-4 lg:grid-cols-2 2xl:grid-cols-3">
            {data.poses.map((pose) => {
              ...
              return <article key={pose.poseIndex} className="overflow-hidden rounded-2xl border border-outline-variant/35 bg-white shadow-sm">
                ...
                {open && (
                  <div className="space-y-4 border-t border-outline-variant/25 bg-surface-container/25 p-4">
                    <div><p className="text-[10px] font-bold uppercase tracking-wide text-secondary">Prompt</p><p className="mt-2 whitespace-pre-wrap text-xs leading-5 text-on-surface">{asset?.prompt || "Prompt was not recorded."}</p></div>
                    {Array.isArray(asset?.reference_manifest?.references) && asset.reference_manifest.references.length > 0 && (
                      <div>
                        <p className="text-[10px] font-bold uppercase tracking-wide text-secondary">References sent to generator{pose.poseIndex === 3 ? " — true back" : ""}</p>
                        <p className="mt-1 text-[10px] leading-4 text-secondary">This immutable attempt record shows the exact selected source files. {pose.poseIndex === 3 ? "The rear product file is the sole visual authority for this pose." : ""}</p>
                        <ol className="mt-2 space-y-1.5">
                          {asset.reference_manifest.references.map((reference, index) => (
                            <li key={`${reference.hash || reference.filename || reference.role}:${index}`} className="flex flex-wrap items-center gap-x-2 gap-y-1 rounded-lg bg-white px-3 py-2 text-[11px]">
                              <span className="font-bold text-on-surface">{reference.imageNumber || index + 1}. {words(reference.role || "reference")}</span>
                              {reference.authority === "true_back_product_authority" && <span className="rounded-full bg-emerald-50 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide text-emerald-700">Rear authority</span>}
                    ...
                    <div><p className="text-[10px] font-bold uppercase tracking-wide text-secondary">Version history</p><div className="mt-2 space-y-2">{(pose.versions.length ? pose.versions : asset ? [asset] : []).map((version) => <div key={version.id || `${pose.poseIndex}:${version.version_number}`} className="flex items-center justify-between rounded-lg bg-white px-3 py-2 text-[11px]"><span className="font-bold text-on-surface">v{version.version_number}</span><span className="text-secondary">{dateTime(version.generated_at)}</span><span className="capitalize text-secondary">{words(version.approval_status)}</span></div>)}</div></div>
                  </div>
                )}
              </article>;
            })}
