...
| Operational teams and memberships | Normalized organization teams and effective-dated memberships are separate from permission-bearing roles. Four workflow teams are backfilled from existing roles; administrators can create/edit teams, choose members and a lead, and audit every save. | **Implemented; live gate** — atomic service-only RPC, tenant RLS/relationship assertions, Admin Teams UI, and post-deploy role harness. |
| Notification preferences | Tenant-scoped member preference rows are authoritative for handoff delivery; profile updates dual-write the legacy JSON during compatibility rollout, and new members receive a normalized default row automatically. | **Implemented; live gate** — self/admin read policy, service-owned writes, team recipient resolution, and post-deploy RLS probes. |
...
| Admin preview/config/send/resend | Authorized users can configure timezone, local send time, operational recipient team/custom recipients, weekdays, holidays, and inspect/preview/send/resend. Team recipients are active members who have not disabled handoff email; the legacy role slug is retained only as a pre-migration fallback. | **Implemented; live gate** — permission boundary is included in the role harness. |
...
| Requirement | Implemented behavior | Status and evidence |
| --- | --- | --- |
| PostgreSQL/Auth/RBAC/RLS | Existing Firebase identities map to organization memberships; operational teams remain separate from granular catalog permissions; every organization-owned V2 table has tenant RLS and organization-leading indexes. A database trigger rejects cross-organization team, member, work-item, batch, event, asset, handoff, and delivery links even from service-role paths. | **Implemented; live gate** — migration validates historical relationships at deploy time; six-role harness proves same/cross-tenant outcomes after deploy. |
| Realtime | Work items, events, pose versions, handoffs, comments, deliveries, teams, team memberships, and notification preferences are added to the publication and clients retain a low-frequency recovery refresh. | **Implemented; live gate** — verify subscription events in deployed Supabase. |
...
| Audit and notifications | Sensitive actions write audit/events and targeted notifications; normalized member preferences participate in recipient selection while legacy JSON is maintained for stable assignment-alert compatibility. | **Implemented; live gate** — recipient visibility, preference isolation, and addressing are checked by the role harness. |
| Safe migration | The already-applied base migration is immutable. Hardening is additive, validates RLS/policy and tenant-relationship coverage, backfills state/version/handoff/assignment/team/preference data, and deploys before API/UI. | **Implemented** — verifier parses both migrations; the live preflight passed the existing 33 relationships and deployment adds nine team/preference assertions, while Git comparison protects the base file. |
