...
<CardGroup cols={1}>
  ...
  <Card title="Pose 6: Feature Showcase (Chosen Per Product)" icon="shirt">
    **Framing**: Decided by the analysis, not by a template. The vision model inspects the product references, picks the ONE feature a buyer actually judges this product by, and designs this frame around it.
    <br/>**What it can pick**: an embroidered or sequinned yoke, a neckline construction, a sleeve treatment, a print's scale and repeat, the cut and flare of the bottom wear, a hem or border, a dupatta's print/border/length/drape, a saree's pallu, drape style, border or blouse — or any other genuinely distinguishing detail.
    <br/>**Shot type**: also chosen — `macro_detail` (fill the frame), `half_body_detail` (crop to the section it sits on), `full_body_feature` (needs the whole garment: flare, length, fall), `drape_feature` (a held or spread panel), or `movement_feature` (fall and flare in motion).
    <br/>**Non-duplication (enforced)**: it may not be a second hero shot, and it may not re-shoot Pose 5's subject. If the strongest feature is already Pose 5's close-up subject, the frame is automatically widened so it shows how that feature sits on the worn garment instead of re-cropping the same detail.
  </Card>
</CardGroup>
...
## How Pose 6 Is Chosen, and How It Learns

The sixth frame is the only one without a fixed job, and the analysis records its decision in `showcasePlan`: the named feature, its garment region, the shot type, why it sells, the reference that proves it, and one line stating how the frame differs from Poses 1 and 5.

Those decisions are scored by what your team actually does with them:

| Signal | Recorded as |
| --- | --- |
| A reviewer approves the sixth pose version, or approves the whole set | `selected` |
| A reviewer rejects the sixth pose version | `rejected` |
| Anyone asks for the sixth pose to be regenerated | `regenerated` |
| Automatic QA refuses the sixth pose | `qa_failed` |

Counts are kept per organization, category, garment family, feature region and shot type — never per SKU — and fed back into later analyses as a short "what this team kept and threw away" note. It breaks ties between comparably strong features and steers away from a framing your team keeps sending back. It is house taste, not product truth: when the references clearly make a different feature the strongest one, the analysis follows the references.
